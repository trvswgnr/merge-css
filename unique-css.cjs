 ...
 * @param {string} targetCss - The target CSS that we want to keep the unique rules from
 * @param {string} otherCss - The other CSS that should be compared against the target
 * @returns {string} The CSS output with only unique rules from the target
 */
function uniqueCss (targetCss, otherCss) {
 ...
 * @param {CssNode} targetParentNode
 * @param {CssNode} otherParentNode
 * @returns {postcss.ChildNode[]} The unique nodes
 */
function getUniqueNodes (targetParentNode, otherParentNode) {
...
/**
 * @typedef {postcss.Root | postcss.Rule | postcss.AtRule | postcss.ChildNode} CssNode
 */

module.exports = uniqueCss
